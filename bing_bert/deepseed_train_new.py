...
def train(args, index, model, optimizer, worker_init, pool, finetune=False):
    global global_step
    ...
    for f_id in range(f_start_id + 1, len(files)):
        ...
        dataset_future = pool.submit(create_pretraining_dataset, data_file, args.max_predictions_per_seq, shared_file_list, args.train_micro_batch_size_per_gpu, worker_init)
...
def run(args, model, optimizer, start_epoch):
    ...
    for index in range(start_epoch, config["training"]["num_epochs"]):
        ...
        pre = time.time()
        train(args, index, model, optimizer, worker_init, pool)
